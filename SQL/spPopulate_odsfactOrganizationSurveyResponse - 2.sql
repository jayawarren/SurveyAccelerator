...
BEGIN TRAN
	...
	FROM	(
			SELECT	asr.OrganizationSurveyKey SurveyFormKey,
					...
					*/
					STDEVP(bsr.ResponsePercentage) OVER (PARTITION BY LEFT(bsr.GivenDateKey, 4), asr.OrganizationSurveyKey, dq.QuestionKey) StdDevBranchResponsePercentage, 
			...
			FROM	Seer_ODS.dbo.factBranchSurveyResponse bsr
					...
					(SELECT	da.OrganizationKey,
							...
							*/
							STDEVP(asr.ResponsePercentage) OVER (PARTITION BY LEFT(asr.GivenDateKey, 4), asr.SurveyFormKey, dq.QuestionKey) StdDevAssociationResponsePercentage
