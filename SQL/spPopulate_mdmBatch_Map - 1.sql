...
BEGIN TRAN
	...
	USING	(
			SELECT	@change_datetime change_datetime,
					CASE WHEN C.current_date_given_key <> C.date_given_key THEN @change_datetime
					...
					C.batch_key,
					C.date_given_key,
					CASE WHEN C.current_date_given_key <> C.date_given_key THEN 0
						ELSE C.current_indicator
					END current_indicator,
					C.previous_batch_key,
			...
			FROM	(
					SELECT	C.organization_key,
							...
							C.date_given_key,
							1 current_indicator,
							MAX(C.date_given_key) current_date_given_key,
