...
/****** Object:  View [dd].[vwNewMemBranchSurveyResponders]    Script Date: 09/05/2013 20:21:46 ******/
SET ANSI_NULLS ON
GO
...
SELECT	F.association_key AssociationKey,
		...
		F.response_percentage*100 ResponsePercentage
...
FROM	(
		...
		FROM	(
				...
				FROM	(
						...
						/* Temporary code until New Member data is part of the Observation Table data source */
						
						UNION
						
						SELECT	B.official_association_number,
						...
						FROM	Seer_ODS.dbo.Open_Ends B
						
						WHERE	B.current_indicator = 1
								AND B.form_code LIKE '%NEW%'
						
						GROUP BY B.official_association_number,
								B.official_branch_number,
								B.current_indicator,
								B.form_code,
								B.batch_number,
