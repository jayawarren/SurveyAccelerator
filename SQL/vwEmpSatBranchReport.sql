...
CREATE VIEW [dd].[vwEmpSatBranchReport]
AS
WITH newsq AS (			
...
FROM	[dbo].[factBranchStaffExperienceReport] bmsr	  	  
		...
		INNER JOIN allakki_assoc
			ON allakki_assoc.AssociationKey = db1.AssociationKey AND allakki_assoc.GivenDateKey = db1.GivenDateKey AND allakki_assoc.SurveyQuestionKey = bmsr.SurveyQuestionKey AND allakki_assoc.QuestionResponseKey = bmsr.QuestionResponseKey
...
WHERE	GivenDateKey is not NULL				
)
--SELECT * FROM rpt
,aprevrptbase AS 
(
