...
/****** Object:  View [dd].[vwEmpSatBranchDetailSurveyQuestions_Wellbeing]    Script Date: 08/04/2013 08:22:05 ******/
SET ANSI_NULLS ON
GO
...
--SELECT * FROM [dd].[vwEmpSatBranchDetailSurveyQuestions_WellBeing] WHERE associationname = 'YMCA of Central Stark County' /*and Question like 'overall, how would you rate%'*/ order by 2,3,4
--SELECT * FROM [dd].[vwEmpSatBranchDetailSurveyQuestions_WellBeing] WHERE associationname = 'YMCA of Central Stark County' and Branch like 'Lake Community%' and Question like 'overall, how would you rate%' order by 2,3,4
--SELECT * FROM [dd].[vwEmpSatBranchDetailSurveyQuestions_WellBeing] WHERE associationname like '%seattle%' order by 2,3,4
ALTER VIEW [dd].[vwEmpSatBranchDetailSurveyQuestions_WellBeing] 
AS 
WITH newsq AS (			
...
WHERE	dc.Category NOT IN ('Survey Responders')
		AND dc.CategoryType = 'YUSA'
)
,newsqmapped AS (
SELECT	distinct
...
FROM	dbo.factBranchMemberSatisfactionReport msr
		INNER JOIN dd.factEmpSatDashboardBase db
			ON db.BranchKey = msr.BranchKey
				and db.batch_key = msr.batch_key
