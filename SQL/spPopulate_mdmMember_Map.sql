...
BEGIN
	...
	USING	(
			SELECT	B.member_map_key,
					C.member_key,
					C.member_key matching_member_key,
					C.seer_key,
					D.organization_key,
					COALESCE(E.program_group_key, -1) program_group_key,
			...
			FROM	Seer_MDM.dbo.Member A
					...
					LEFT JOIN Seer_MDM.dbo.Program_Group E
						ON A.program_site_location = E.program_site_location
