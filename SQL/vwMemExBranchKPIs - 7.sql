...
,KPI_Loyalty_Crt_vals AS (
	SELECT	distinct
			...
			db.batch_key,
			db.GivenDateKey,
			bmsr.BranchPercentage,
			bmsr.AssociationPercentage,
			bmsr.NationalPercentage,
			CASE WHEN bmsr.PreviousBranchPercentage = 0 then NULL
				ELSE bmsr.[PreviousBranchPercentage]
			END AS [PreviousBranchPercentage]
...
,KPI_HS_Crt_vals AS (
	SELECT	distinct
			...
			db.batch_key,
			db.GivenDateKey,
			COALESCE(A.measure_value, bmsr.BranchPercentage) BranchPercentage,
			COALESCE(C.measure_value, bmsr.AssociationPercentage) AssociationPercentage,
			COALESCE(D.measure_value, bmsr.NationalPercentage) NationalPercentage,
			CASE WHEN bmsr.PreviousBranchPercentage = 0 then NULL
				ELSE bmsr.[PreviousBranchPercentage]
			END AS [PreviousBranchPercentage]
	...
	FROM	[dbo].[factBranchMemberSatisfactionReport] bmsr
			...
			LEFT JOIN Seer_ODS.dbo.Top_Box C
				on db.AssociationNumber = C.official_association_number
					and db.OfficialBranchNumber = C.official_branch_number
					and  sf.survey_form_code = C.form_code
					and LOWER(REPLACE(bmsr.Segment, ' ', '')) = REPLACE(C.measure_type, 'healthseekers', 'healthseeker')
			LEFT JOIN Seer_ODS.dbo.Top_Box D
				on sf.survey_form_code = D.form_code
					and LOWER(REPLACE(bmsr.Segment, ' ', '')) = REPLACE(D.measure_type, 'healthseekers', 'healthseeker')
	
	WHERE 	bmsr.Segment = 'Health Seeker'
			and COALESCE(A.aggregate_type, 'Branch') = 'Branch'
	...
	FROM	[dbo].[factBranchMemberSatisfactionReport] bmsr
			...
			LEFT JOIN Seer_ODS.dbo.Top_Box C
				on db.AssociationNumber = C.official_association_number
					and db.OfficialBranchNumber = C.official_branch_number
					and  sf.survey_form_code = C.form_code
					and LOWER(REPLACE(REPLACE(bmsr.Segment, ' ', ''), 'Fitness', 'Meeting')) = REPLACE(REPLACE(C.measure_type, '_', ''), 'healthseekers', 'healthseeker')	
			LEFT JOIN Seer_ODS.dbo.Top_Box D
				on sf.survey_form_code = D.form_code
					and LOWER(REPLACE(REPLACE(bmsr.Segment, ' ', ''), 'Fitness', 'Meeting')) = REPLACE(REPLACE(D.measure_type, '_', ''), 'healthseekers', 'healthseeker')
...
--select * from KPI_HealthSeekers_Crt_vals
	...
	FROM	[dbo].[factBranchMemberSatisfactionReport] bmsr
			...
			LEFT JOIN Seer_ODS.dbo.Top_Box C
				on db.AssociationNumber = C.official_association_number
					and db.OfficialBranchNumber = C.official_branch_number
					and  sf.survey_form_code = C.form_code
					and LOWER(REPLACE(REPLACE(bmsr.Segment, ' ', ''), 'Fitness', 'Meeting')) = REPLACE(REPLACE(REPLACE(C.measure_type, '_', ''), 'healthseekers', 'healthseeker'), '-', '')		
			LEFT JOIN Seer_ODS.dbo.Top_Box D
				on sf.survey_form_code = D.form_code
					and LOWER(REPLACE(REPLACE(bmsr.Segment, ' ', ''), 'Fitness', 'Meeting')) = REPLACE(REPLACE(REPLACE(D.measure_type, '_', ''), 'healthseekers', 'healthseeker'), '-', '')
	
	WHERE	bmsr.Segment = 'Non Health Seeker Fitness Goals'
			and COALESCE(A.aggregate_type, 'Branch') = 'Branch'
			and COALESCE(C.aggregate_type, 'Association') = 'Association'
			and COALESCE(D.aggregate_type, 'National') = 'National'
			and bmsr.current_indicator = 1
...
FROM	(
		...
		SELECT	db.AssociationKey,
				...
				CASE
				 ...
				 END AS SortOrder,
				(COALESCE(promoter.BranchPercentage, 0.000000) - COALESCE(detractor.BranchPercentage, 0.000000)) AS BranchPercentage,
				(COALESCE(apromoter.AssociationPercentage, 0.000000) - COALESCE(adetractor.AssociationPercentage, 0.000000)) AS AssociationPercentage,
				(COALESCE(apromoter.NationalPercentage, 0.000000) - COALESCE(adetractor.NationalPercentage, 0.000000)) AS NationalPercentage,
				(COALESCE(promoter.PreviousBranchPercentage, 0.000000) - COALESCE(detractor.PreviousBranchPercentage, 0.000000)) AS PreviousBranchPercentage,	
		...
		FROM	dd.factMemExDashboardBase db
				LEFT JOIN KPI_HS_Crt_vals
					ON db.BranchKey = KPI_HS_Crt_vals.BranchKey
						AND db.batch_key = KPI_HS_Crt_vals.batch_key
						AND db.GivenDateKey = KPI_HS_Crt_vals.GivenDateKey
		...
		FROM	dd.factMemExDashboardBase db
				LEFT JOIN KPI_HealthSeekers_Crt_vals
					on db.BranchKey = KPI_HealthSeekers_Crt_vals.BranchKey
						AND db.batch_key = KPI_HealthSeekers_Crt_vals.batch_key
						AND db.GivenDateKey = KPI_HealthSeekers_Crt_vals.GivenDateKey
		...
		FROM	dd.factMemExDashboardBase db
				LEFT JOIN KPI_NonHealthSeekers_Crt_vals
					on db.BranchKey = KPI_NonHealthSeekers_Crt_vals.BranchKey
						AND db.batch_key = KPI_NonHealthSeekers_Crt_vals.batch_key
						AND db.GivenDateKey = KPI_NonHealthSeekers_Crt_vals.GivenDateKey
...
SELECT	AssociationKey,
		...
		CASE WHEN Category = 'Overall Satisfaction' then 1
			...
			WHEN Category IN ('Net Outcome - Individual', 'RPI Individual') then 3
			WHEN Category IN ('Net Outcome - Community', 'RPI Community') then 4
