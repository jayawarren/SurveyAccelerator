...
,KPI_HS_Crt_vals AS (
	SELECT	distinct
			...
			db.SurveyYear,
			db.GivenDateKey,
			COALESCE(A.measure_value, bmsr.AssociationPercentage) AssociationPercentage,
			COALESCE(C.measure_value, bmsr.NationalPercentage) NationalPercentage,
			CASE WHEN bmsr.PreviousAssociationPercentage = 0 then NULL
				ELSE bmsr.[PreviousAssociationPercentage]
			END AS [PreviousAssociationPercentage]
	...
	FROM	[dbo].[factAssociationMemberSatisfactionReport] bmsr
			...
			INNER JOIN Seer_MDM.dbo.Organization B
				ON db.AssociationKey = B.organization_key
			...
			LEFT JOIN Seer_ODS.dbo.Top_Box C
				on sf.survey_form_code = C.form_code
					and LOWER(REPLACE(bmsr.Segment, ' ', '')) = REPLACE(C.measure_type, 'healthseekers', 'healthseeker')
			
	WHERE 	bmsr.Segment = 'Health Seeker'
			and COALESCE(A.aggregate_type, 'Association') = 'Association'
			and COALESCE(C.aggregate_type, 'National') = 'National'
			and bmsr.current_indicator = 1
			and db.current_indicator = 1
