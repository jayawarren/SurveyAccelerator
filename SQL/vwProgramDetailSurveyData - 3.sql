...
FROM	(
		...
		FROM	dbo.dimSurveyQuestion sq
				...
				INNER JOIN [dbo].[factProgramGroupProgramReport] bmsr	 
					ON amsr.SurveyFormKey = bmsr.OrganizationSurveyKey
						AND amsr.SurveyQuestionKey = bmsr.SurveyQuestionKey
						AND amsr.QuestionResponseKey = bmsr.QuestionResponseKey
						AND amsr.batch_key = bmsr.batch_key
						AND amsr.GivenDateKey = bmsr.GivenDateKey
				INNER JOIN dd.factProgramDashboardBase db1		
					ON amsr.SurveyFormKey = db1.SurveyFormKey
		...
		WHERE	amsr.current_indicator = 1
