	...
	FROM	(
			...
			FROM	dbo.factBranchSurveyResponse bsr
					...
					FROM 	dbo.factAssociationSurveyResponse asr
							...
							INNER JOIN
							(
							SELECT	LEFT(asr.GivenDateKey, 4) Year,
									asr.SurveyFormKey,
									dq.QuestionKey,
									dr.ResponseKey,
									CONVERT(DECIMAL(19, 6), STDEVP(asr.ResponsePercentage)) StdDevAssociationResponsePercentage
							...
							GROUP BY LEFT(asr.GivenDateKey, 4),
									asr.SurveyFormKey,
									dq.QuestionKey,
									dr.ResponseKey
							) A
							ON LEFT(asr.GivenDateKey, 4) = A.Year
								AND asr.SurveyFormKey = A.SurveyFormKey
								AND dq.QuestionKey = A.QuestionKey
								AND dr.ResponseKey = A.ResponseKey
			...
			FROM	(
					SELECT	asr.OrganizationSurveyKey SurveyFormKey,
							...
							END  AS DECIMAL(6,5)) StdDevBranchResponsePercentage,
							*/
							STDEVP(bsr.ResponsePercentage) StdDevBranchResponsePercentage,
							MAX(asr.StdDevAssociationResponsePercentage) StdDevAssociationResponsePercentage
